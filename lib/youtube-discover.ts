...
export function categoryLabel(categoryId: number): string {
  return YOUTUBE_VIDEO_CATEGORIES.find((c) => c.id === categoryId)?.label ?? `Category ${categoryId}`
}
...
/**
 * Pull YouTube's "most popular" chart for one region + category.
 * Costs 1 YouTube API unit per call (up to 50 videos returned).
 */
...
/** Fetch trending for many categories sequentially (respects API pacing). */
